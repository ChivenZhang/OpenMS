...
Property::Property()
{
	...
	if (std::filesystem::exists(configFile) == false) MS_FATAL("config file not found: {}", configFile.c_str());

	// Load config file

	std::ifstream ifs(configFile);
	if (!ifs.is_open()) MS_FATAL("Failed to open config file: {}", configFile.c_str());
	...
	parse_func = [&](MSString const& name, uint32_t depth, nlohmann::ordered_json& value) {
		...
		case nlohmann::ordered_json::value_t::boolean:
			m_PropertyMap[name] = MSTextC::to_string(value.get<bool>());
			break;
		...
		case nlohmann::ordered_json::value_t::number_float:
			m_PropertyMap[name] = MSTextC::to_string(value.get<float>());
			break;
		case nlohmann::ordered_json::value_t::number_integer:
			m_PropertyMap[name] = MSTextC::to_string(value.get<int32_t>());
			break;
		case nlohmann::ordered_json::value_t::number_unsigned:
			m_PropertyMap[name] = MSTextC::to_string(value.get<uint32_t>());
			break;
		default:
			break;
		}
