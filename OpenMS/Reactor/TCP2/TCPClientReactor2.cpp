...
*
* =================================================*/
#include "TCPClientReactor2.h"
#include "TCPChannel2.h"
#include <asio.hpp>

TCPClientReactor2::TCPClientReactor2(MSRef<ISocketAddress> address, size_t workerNum, callback_tcp_t callback)
	:
...
void TCPClientReactor2::startup()
{
	...
	m_EventThread = MSThread([=, &promise]()
	{
		...
		do
		{
			...
			client.async_connect(tcp::endpoint(address::from_string(m_Address->getAddress()), m_Address->getPort()), [&](asio::error_code error)
			{
				if (error) MS_ERROR("failed to connect: %s", error.message().c_str());
				else
				{
...
void TCPClientReactor2::shutdown()
{
...
MSHnd<IChannelAddress> TCPClientReactor2::address() const
{
	return m_Connect ? m_LocalAddress : MSHnd<IChannelAddress>();
}

void TCPClientReactor2::write(MSRef<IChannelEvent> event)
{
	if (m_Running == false) return;
...
void TCPClientReactor2::onConnect(MSRef<Channel> channel)
{
...
void TCPClientReactor2::onDisconnect(MSRef<Channel> channel)
{
...
void TCPClientReactor2::onOutbound(MSRef<IChannelEvent> event, bool flush)
{
