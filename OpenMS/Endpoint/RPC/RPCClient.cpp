...
bool RPCClientBase::call(MSStringView const& name, uint32_t timeout, MSStringView const& input, MSString& output)
{
	...
	// MS_INFO("客户发送请求:地址 {} 会话 {} 长度 {} 内容 {}", m_Reactor->address().lock()->getString().c_str(), request.Session, request.Length, input.data());
...
bool RPCClientInboundHandler::channelRead(MSRaw<IChannelContext> context, MSRaw<IChannelEvent> event)
{
	...
	if (sizeof(RPCRequestBase) <= m_Buffer.size() && package.Length <= m_Buffer.size())
	{
		...
		if (package.Session & 0X80000000)
		{
			...
			// MS_INFO("客户接收请求:地址 {} 会话 {} 长度 {} 内容 {}", m_Client->m_Reactor->address().lock()->getString().c_str(), package.Session, package.Length, message.data());
			MSString output;
		...
		else
		{
			...
			// MS_INFO("客户接收响应:地址 {} 会话 {} 长度 {} 内容 {}", m_Client->m_Reactor->address().lock()->getString().c_str(), package.Session, package.Length, message.data());
