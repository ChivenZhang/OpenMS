...
bool RPCServerBase::call(MSHnd<IChannel> client, MSStringView const& name, uint32_t timeout, MSStringView const& input, MSString& output)
{
	...
	// MS_INFO("服务发送请求:地址 {} 会话 {} 长度 {} 内容 {}", m_Reactor->address().lock()->getString().c_str(), request.Session, request.Length, input.data());
...
bool RPCServerInboundHandler::channelRead(MSRaw<IChannelContext> context, MSRaw<IChannelEvent> event)
{
	...
	if (sizeof(RPCRequestBase) <= m_Buffer.size() && package.Length <= m_Buffer.size())
	{
		...
		if (package.Session & 0X80000000)
		{
			...
			// MS_INFO("服务接收响应:地址 {} 会话 {} 长度 {} 内容 {}", m_Server->m_Reactor->address().lock()->getString().c_str(), package.Session, package.Length, message.data());
		...
		else
		{
			...
			// MS_INFO("服务接收请求:地址 {} 会话 {} 长度 {} 内容 {}", m_Server->m_Reactor->address().lock()->getString().c_str(), package.Session, package.Length, message.data());
			if (message.size() <= m_Server->m_Config.Buffers)
			{
				MSString output;
