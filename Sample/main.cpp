...
int main()
{
	if (false)
	{
		TCPServerReactor server(IPv4Address::New("0.0.0.0", 6000), 128, 2, {
		...
		TCPClientReactor client(IPv4Address::New("127.0.0.1", 6000), 1, {
			[](TRef<IChannel> channel) {	// Connected
				auto ipv4 = TCast<IPv4Address>(channel->getRemote());
				TPrint("connect %s:%d", ipv4->getAddress().c_str(), ipv4->getPort());
	...
	if (false)
	{
		...
		UDPClientReactor client(IPv4Address::New("127.0.0.1", 6000), false, false, 1, {
			[](TRef<IChannel> channel) {	// Connected
				auto ipv4 = TCast<IPv4Address>(channel->getRemote());
				TPrint("connect %s:%d", ipv4->getAddress().c_str(), ipv4->getPort());
	...
	if (true)
	{
		...
		KCPClientReactor client(IPv4Address::New("127.0.0.1", 6000), 1, {
			[](TRef<IChannel> channel) {	// Connected
				auto ipv4 = TCast<IPv4Address>(channel->getRemote());
				TPrint("connect %s:%d", ipv4->getAddress().c_str(), ipv4->getPort());
